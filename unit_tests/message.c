...
int log_mainqueue(void){
	...
	if(mq_receive(main_to_log, (char*)&rmsg_main, \
    	...
    	if(rmsg_main.data == LIGHT_TASK && rmsg_main.log_level == ALERT){
    		...
    		fwrite(data, sizeof(char), strlen(data), file);
    		sprintf(data, "Time : %ld secs, %ld usecs | Source : MAIN task | Terminating Light task\n",\
    				rmsg_main.time_stamp.tv_sec, rmsg_main.time_stamp.tv_usec);
    		fwrite(data, sizeof(char), strlen(data), file);
		...
		/* checks of the log_level is alert and its from TEMP_TASK*/
    		...
    		sprintf(data, "Time : %ld secs, %ld usecs | Source : MAIN task | TMP 102 I2C Failed\n",\
    				rmsg_main.time_stamp.tv_sec, rmsg_main.time_stamp.tv_usec);
    		fwrite(data, sizeof(char), strlen(data), file);
    		sprintf(data, "Time : %ld secs, %ld usecs | Source : MAIN task | Terminating Temperature task\n",\
    				rmsg_main.time_stamp.tv_sec, rmsg_main.time_stamp.tv_usec);
    		fwrite(data, sizeof(char), strlen(data), file);
    		temp_alive = 1;
		 ...
		 * */
    		...
    		sprintf(data, "Time : %ld secs, %ld usecs | Source : MAIN task | No TEMP Heart beat\n",\
    				rmsg_main.time_stamp.tv_sec, rmsg_main.time_stamp.tv_usec);
    		fwrite(data, sizeof(char), strlen(data), file);
    		sprintf(data, "Time : %ld secs, %ld usecs | Source : MAIN task | Terminating Temperature task\n",\
    				rmsg_main.time_stamp.tv_sec, rmsg_main.time_stamp.tv_usec);
...
int log_init(){
	...
	int status= 0;
	status = sprintf(data, "Time : %ld secs, %ld usecs | Source : Log task | Log task initialised\n",\
